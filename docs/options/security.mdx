...
<TabItem value="cli" label="Auth0 CLIを使用してOIDCアプリを作成する">

#### Auth0 CLIを使用してOIDCアプリを作成する

1. Auth0アプリケーションを作成し、`.auth0.env`を生成する
    ```shell
...
3. ロールを作成する (JHipsterは`ROLE_ADMIN`と`ROLE_USER`を想定しています)
    ```shell
...
6. 「ロールを追加」アクションを作成、デプロイし、ログインにバインドする
