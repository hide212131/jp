...
JHipsterビルドでデフォルトの依存関係チェックを提案することは、何度か議論されてきました（[#6329](https://github.com/jhipster/generator-jhipster/issues/6329)、[#8191](https://github.com/jhipster/generator-jhipster/issues/8191)）。要約すると、現実的なレポート（誤検知の除去）を作成することは複雑であり、コンテキストに依存します（セキュリティは常に実際の危険性/重要性とそれを防ぐための努力との間のトレードオフです）。
...
OWASPプロジェクトは、MavenおよびGradleプラグインを提供しており、依存関係チェーン全体を自動的にチェックし、レポートを生成し、ビルドをブロックすることさえ可能です（継続的インテグレーションを行う際には非常に攻撃的になる可能性があるため、推奨されません）。
