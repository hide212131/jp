...
[OWASP Top 10 Most Critical Web Application Security Risks](https://www.owasp.org/index.php/Category:OWASP_Top_Ten_Project)によると、既知の脆弱性を持つコンポーネント（すなわち依存関係）の使用は9位にランクされており、悪意のある、またはそうでないサードパーティの依存関係によって引き起こされたセキュリティ侵害の事例が多数知られています。
