...
* ___2.1.4___ `src\main\webapp\app\home\home.component.html` へ移動し、以下のブロックを削除してください。
```html
...
* ___2.1.9___ `src\test\javascript\spec\app\account`
  - フォルダ `register` 全体を削除します。これには `register.component.spec.ts` が含まれます。

### 2.2 Vue

* ___2.2.1___ フォルダ `src/main/webapp/app/account/register` 全体を削除します。これには `register.component.spec.ts`、`register.component.ts`、`register.service.ts`、`register.vue` が含まれます。
