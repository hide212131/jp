...
__[@apuntandoanulo](https://github.com/apuntandoanulo) がヒントを提供し、[@juandbc](https://github.com/juandbc) が更新しました。__
...
* ___2.1.2___ `src\main\webapp\app\account\account.module.ts` を開き、以下の行を削除します。
...
* ___2.1.5___ `src\main\webapp\app\layouts\navbar\navbar.component.html` に移動し、以下のブロックを削除します。
```html
...
```ts
* ___2.1.7___ `src\main\webapp\app\shared\login\login.component.ts` に移動し、以下のブロックを削除します。
...
* ___2.1.8___ メッセージファイルを削除: `src\main\webapp\i18n\ ... \register.json`

* ___2.1.9___ `src\test\javascript\spec\app\account`
  - `register.component.spec.ts` を含むフォルダー `register` 全体を削除

### 2.2 Vue

* ___2.2.1___ `register.component.spec.ts`、`register.component.ts`、`register.service.ts`、`register.vue` を含むフォルダー `src/main/webapp/app/account/register` 全体を削除
...
* ___2.2.5___ メッセージファイル `src/main/webapp/i18n/.../register.json` を削除

* ___2.2.6___ テストファイル `src/test/javascript/cypress/e2e/account/register-page.cy.ts` を削除
