...
__ヒント提供者: [@apuntandoanulo](https://github.com/apuntandoanulo) と 更新者: [@juandbc](https://github.com/juandbc)__
...
* ___2.1.2___ `src\main\webapp\app\account\account.module.ts` を開き、以下の行を削除してください。
  - _declarations_ array -> `RegisterComponent`

* ___2.3___ `src\main\webapp\app\account\account.route.ts`に移動して、次の行を削除します。
* ___2.1.3___ `src\main\webapp\app\account\account.route.ts`を開いて、以下の行を削除してください:
  - _ACCOUNT_ROUTES_ array -> `registerRoute `

* ___2.4___ `src\main\webapp\app\home\home.component.html`に移動し、次のブロックを削除します。
* ___2.1.4___ `src\main\webapp\app\home\home.component.html`に移動し、以下のブロックを削除します:
```html
