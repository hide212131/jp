...
* ___2.1.9___ `src\test\javascript\spec\app\account`
  - `register` フォルダー（`register.component.spec.ts` を含む）全体を削除します。

### 2.2 Vue

* ___2.2.1___ `src/main/webapp/app/account/register` フォルダー（`register.component.spec.ts`、`register.component.ts`、`register.service.ts`、`register.vue` を含む）全体を削除します。
