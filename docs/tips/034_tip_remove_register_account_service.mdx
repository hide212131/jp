...
__[@apuntandoanulo](https://github.com/apuntandoanulo) 提供のヒント & [@juandbc](https://github.com/juandbc) による更新__
...
* ___2.1.6___ `src\main\webapp\app\shared\login\login.component.html` に移動し、以下のブロックを削除します。
...
```ts
* ___2.1.7___ `src\main\webapp\app\shared\login\login.component.ts` を開き、以下のブロックを削除してください。
...
* ___2.1.9___ `src\test\javascript\spec\app\account`
  - `register`フォルダー全体を削除（含まれるもの: `register.component.spec.ts`）

### 2.2 Vue

* ___2.2.1___ `src/main/webapp/app/account/register`フォルダー全体を削除（含まれるもの: `register.component.spec.ts`, `register.component.ts`, `register.service.ts`, `register.vue`）
