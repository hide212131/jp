...
AOPを使用すると、ロギング、セキュリティ、トランザクション管理などの横断的関心事を、コアビジネスロジックを変更することなく導入できます。この関心事の分離により、よりクリーンでモジュール化されたコードの維持に役立ちます。
...
サービス・メソッドの複数の実装を持つ可能性が低い場合や、サービス・メソッドの頻繁な変更を予想しない場合は、ステップ1を省略し、このアプローチの簡略化された実装のために、以下のステップで関連する変更を調整できます。
