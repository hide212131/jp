...
すべてのカスタムメソッドを含むリポジトリインターフェースを、別のカスタムパッケージ内に作成します。このアプローチにより、生成されたリポジトリコードは変更されず、カスタムロジックは整理され、保守しやすくなります。
...
AOPを使用すると、コアビジネスロジックを変更することなく、ロギング、セキュリティ、トランザクション管理などの横断的関心事を導入できます。この関心事の分離により、よりクリーンでモジュール化されたコードを維持できます。
...
サービス・メソッドの複数の実装を持つ可能性が低い場合や、サービス・メソッドの頻繁な変更を想定しない場合は、ステップ1を省略し、このアプローチの簡略化された実装のために、以下のステップで関連する変更を調整できます。
